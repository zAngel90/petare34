...
import { Check, ShoppingCart, CreditCard, CheckCircle, X, Gamepad2 } from 'lucide-react';
...
import './CheckoutRobux.css';

function CheckoutRobux() {
  const navigate = useNavigate();
  const { items: cart, clearCart } = useCart();
  const { user, isAuthenticated } = useAuth();

  // Leer datos del paquete seleccionado desde Robux.jsx
  ...
  // Cargar paquete seleccionado desde localStorage
  useEffect(() => {
    const savedPackage = localStorage.getItem('selectedRobuxPackage');
    if (savedPackage) {
      const pkg = JSON.parse(savedPackage);
      setSelectedPackage(pkg);
    } else {
      // Si no hay paquete, redirigir a /robux
      navigate('/robux');
    }
  }, [navigate]);
  ...
  // Usar datos del paquete seleccionado
  const totalRobux = selectedPackage?.amount || 0;
  const totalPrice = selectedPackage?.price || 0;

  // Avanzar al siguiente paso
  const handleNextStep = () => {
    // Obtener el método de entrega
    const deliveryMethod = selectedPackage?.deliveryMethod || 'gamepass';
    ...
    if (currentStep === 1) {
      ...
      // Validar gamepass si el método es "gamepass"
      if (deliveryMethod === 'gamepass' && !selectedGamepass) {
        alert('Por favor selecciona un gamepass');
        return;
      }
      
      // Validar grupo si el método es "group" o "grupo"
      if ((deliveryMethod === 'group' || deliveryMethod === 'grupo') && !selectedGamepass) {
        alert('Por favor verifica tu membresía en algún grupo');
        return;
      }
    }
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        productDetails: {
          gamepassId: selectedGamepass?.id || null,
          gamepassName: selectedGamepass?.name || null,
          gamepassPrice: selectedGamepass?.price || null,
          placeId: selectedGamepass?.placeId || null, // ✅ Agregado para RBXCrate
          robuxAmount: totalRobux
        },
        amount: totalRobux,
        price: totalPrice,
        currency: 'PEN',
      ...
      if (data.success) {
        localStorage.removeItem('selectedRobuxPackage');
        navigate('/orders');
      } else {
  ...
  return (
    <div className="checkout-robux-page checkout-items-page">
      <div className="checkout-container">
        <h1>Checkout - Robux</h1>

        <div className="checkout-robux-horizontal">
          {/* Columna Izquierda: Steps y Formulario */}
          <div className="checkout-robux-left">
            {/* Progress Steps */}
          ...
          <div className={`step ${currentStep >= 3 ? 'active' : ''}`}>
            ...
            {/* Step 1: Verificar Usuario y Gamepass/Grupo */}
            {currentStep === 1 && (
          <div className="step-content">
            <h2 className="step-title">
              {(selectedPackage?.deliveryMethod === 'group' || selectedPackage?.deliveryMethod === 'grupo')
                ? 'Verificar Usuario y Grupo' 
                : 'Verificar Usuario y Gamepass'}
            </h2>
            <p className="step-description">
              {(selectedPackage?.deliveryMethod === 'group' || selectedPackage?.deliveryMethod === 'grupo')
                ? 'Busca tu usuario de Roblox y verifica tu membresía en los grupos'
                : 'Busca tu usuario de Roblox y selecciona el gamepass para recibir los Robux'}
            </p>
            
            <div className="verification-section">
              <h3 className="section-subtitle">Usuario de Roblox</h3>
              <RobloxUserSearch 
            ...
            {selectedUser && selectedPackage?.deliveryMethod === 'gamepass' && (
              <div className="verification-section" style={{ marginTop: '30px' }}>
                <h3 className="section-subtitle">Gamepass para Transferencia</h3>
                <GamePassSelector
                  userId={selectedUser.id}
                  selectedAmount={totalRobux}
            ...
            {selectedUser && (selectedPackage?.deliveryMethod === 'group' || selectedPackage?.deliveryMethod === 'grupo') && (
              <div className="verification-section" style={{ marginTop: '30px' }}>
                <h3 className="section-subtitle">Verificación de Grupo</h3>
                <CommunityVerification 
                  robloxUserId={selectedUser.id}
                  robloxUsername={selectedUser.name}
                  onVerificationChange={(data) => {
                    console.log('🔍 Verificación recibida:', data);
                    // Cuando se verifica, marcar como "grupo verificado" para habilitar el botón
                    // Aceptar si está registrado en los grupos (isRegistered) aunque no tenga 14 días completos
                    if (data && (data.isRegistered || data.isFullyVerified || data.verified || data.isVerified)) {
                      console.log('✅ Usuario verificado en grupos - Entrega:', data.deliveryType);
                      console.log('⏰ Tiempo de entrega:', data.deliveryTime);
                      // Crear un objeto simulado de "grupo" para que el botón se habilite
                      setSelectedGamepass({
                        id: 'group-verified',
                        name: `Grupos Verificados (${data.deliveryTime})`,
                        verified: true,
                      ...
                      console.log('❌ Usuario NO está registrado en los grupos');
            ...
            <div style={{ marginTop: '40px' }}>
              <button 
                ...
                disabled={!selectedUser || !selectedGamepass}
              >
        ...
        {currentStep === 2 && (
          <div className="step-content">
            ...
            <div className="verified-info-cards">
              <div className="verified-user-info">
                <Check size={16} />
                <span>Usuario: <strong>{selectedUser.name}</strong></span>
              </div>
              {selectedGamepass && selectedPackage?.deliveryMethod === 'gamepass' && (
                <div className="verified-gamepass-info">
                  <span>Gamepass: <strong>{selectedGamepass.name}</strong></span>
                </div>
              )}
              
              {selectedGamepass && (selectedPackage?.deliveryMethod === 'group' || selectedPackage?.deliveryMethod === 'grupo') && (
                <div className="verified-gamepass-info">
                  <span>Grupo: <strong>{selectedGamepass.name}</strong></span>
                </div>
              )}
            </div>
        ...
        {currentStep === 3 && (
          <div className="step-content">
            ...
            <div className="confirmation-details">
              ...
              {selectedGamepass && selectedPackage?.deliveryMethod === 'gamepass' && (
                <div className="detail-item">
                  <span className="detail-label">Gamepass:</span>
                  <span className="detail-value">{selectedGamepass.name}</span>
                </div>
              )}
              ...
              <div className="detail-item">
                <span className="detail-label">Total Robux:</span>
                <span className="detail-value">{totalRobux.toLocaleString()} Robux</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">Total a Pagar:</span>
                <span className="detail-value total">S/ {totalPrice.toFixed(2)}</span>
              </div>
            </div>
          ...
          <div className="checkout-robux-right">
            {/* Resumen del pedido - siempre visible */}
            <div className="order-summary">
              <h2><ShoppingCart size={20} /> Resumen del Pedido</h2>
              <div className="robux-summary">
                <div className="robux-total-amount">
                  <Gamepad2 size={32} />
                  <div>
                    <div className="robux-amount">{totalRobux.toLocaleString()} Robux</div>
                    <div className="robux-label">Total de Robux</div>
                  </div>
                </div>
